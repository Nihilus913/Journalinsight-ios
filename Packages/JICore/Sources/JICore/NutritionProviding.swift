...
public protocol NutritionProviding: Sendable {
    /// `GET /api/v1/nutrition/daily?date=` — the meal-timeline detail for one day. `nil` only when
    /// the hub genuinely has no row for that date (never a zeroed-out `NutritionDayDetail`).
