        ...
        List {
            Section {
                VStack(spacing: 4) {
                    ...
                    Text(model.appName).font(.title2.weight(.heavy)).foregroundStyle(JIColor.text)
                    Text(model.versionLine).font(.footnote).foregroundStyle(JIColor.muted)
                        .accessibilityIdentifier("version.line")
                    Text(model.bundleId).font(.caption2).foregroundStyle(JIColor.muted)
                }
...
struct ChangelogEntryRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("\(entry.version) — \(entry.title)").font(.subheadline.weight(.bold)).foregroundStyle(JIColor.text)
            Text(entry.date).font(.caption2).foregroundStyle(JIColor.muted)
            ForEach(Array(entry.items.enumerated()), id: \.offset) { _, item in
                HStack(alignment: .top, spacing: 8) {
                    Text("•").font(.caption).foregroundStyle(JIColor.info)
                    Text(item).font(.caption).foregroundStyle(JIColor.muted)
                }
            }
        }
        .padding(.vertical, 4)
...
private struct AppIconImage: View {
    ...
    private var placeholder: some View {
        RoundedRectangle(cornerRadius: 18, style: .continuous)
            .fill(JIColor.surface2)
            .overlay(Image(systemName: "book.closed.fill").font(.title).foregroundStyle(JIColor.info))
    }
