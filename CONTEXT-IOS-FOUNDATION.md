...
**B-33 Phase C (2026-09-22):** classic removed — `JITheme` is single-case (`.native`, and the `\.jiTheme` environment default), colours only via `theme.color(role)`, radii via `theme.radius(role)`; `JIColor`/`JIPalette` no longer exist, nor do `JIRadius`, `Color(hex:)`/`Color(dark:light:)` or `JIColor.color(for:)` (the hex table and the `JIRadius` line above are history, not live API). The tone → role map lives in `verdictColorRole(_:)` (JIFeatures) and, for the sleep score, `sleepScoreColorRole(score:sourceMissing:)` (JIDesign, nonisolated); `DriverBars.barRole` / `SleepScoreComponents.barRole` / `ContributorBreakdown.barRole` replace the former `Color` constants. **Signature change:** `Surface.init` drops `radius:` — `public init(level: Int = 1, padding: CGFloat = 16, @ViewBuilder content: () -> Content)`; the radius has been level-driven (26/18/12) since §2. `ReadinessArcGauge` loses its classic body and needle; the gauge is the native single-track arc with head dot. `JIScheme` stays (`ThemePrefs.ThemeMode.resolvedScheme(system:)` still uses it).
